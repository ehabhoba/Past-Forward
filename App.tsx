...
function App() {
        ...
        if (image?.status === 'done' && image.url && navigator.share) {
            try {
                ...
                await navigator.share({
                    title: 'Past Forward',
                    text: `Check out my ${decade} look from Past Forward! Created by ehabgm.online`,
                    files: [file],
                });
            } catch (error) {
                console.error('Error sharing image:', error);
                alert("Sharing failed. Your browser might not support sharing files.");
            }
        } else if (image?.status !== 'done') {
            alert("Please wait for the image to finish generating before sharing.");
        } else {
             alert("Your browser doesn't support the Web Share API for files.");
        }
    };
    ...
    const handleDownloadAlbum = async () => {
        ...
        try {
            // Fix: Explicitly type the 'image' parameter in the filter and reduce callbacks to resolve TypeScript inference error.
